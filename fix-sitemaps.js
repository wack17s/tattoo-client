...
const fixSiteMaps = async () => {
  fs.readFile('./public/sitemap.xml', (err, data) => {
    const uaSiteMap = data.toString();
    const ruSiteMap = data.toString();

    fs.writeFile('./public/sitemap-ua.xml', uaSiteMap.replace(/\/ru\//gi, '/ua/').replace(new RegExp(toExclude, 'g'), ''), () => {
      //
    });
    fs.writeFile('./public/sitemap.xml', ruSiteMap.replace(/\/ru\//gi, '/'), () => {
      //
    });
  });
}
